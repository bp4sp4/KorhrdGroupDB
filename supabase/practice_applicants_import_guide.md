...
|----------------------|-----------|------|
| `category`           | 구분(페이지) | 아래 4값 중 하나. 비우면 `타과정` |
...
| `status`             | 상태      | `입금완료` / `확인필요` / `추후진행예정` / `재연계` 중 하나. 비우면 `추후진행예정` |
| `counsel_content`    | 상담내용  | |
| `amount`             | 결제금액  | 숫자만(예: `33000`). 비우면 기본 33000 |
| `manager`            | 담당자    | 선택 |

## category 값 (각각 별도 페이지로 노출)

| category 값 | 메뉴(페이지) |
|-------------|--------------|
| `타과정` | 타과정 실습 (보육교사·평생교육사·한국어교원) |
| `사회복지사` | 사회복지사 실습 |
| `완료` | 실습 완료 건 |
| `환불` | 실습 환불 건 |

> 화면 상세 모달의 **구분** 드롭다운에서 행을 다른 페이지로 이동할 수 있습니다(예: 완료/환불 처리).
