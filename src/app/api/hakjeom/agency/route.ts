...
export async function POST(request: NextRequest) {
  try {
    ...
    const {
      category, address, institution_name, contact,
    ...
    await logAction({ user_id: user.id, user_email: user.email, action: 'create', resource: '기관협약', resource_id: String(data.id), detail: `${data.institution_name} 기관 등록` });
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    const body = await request.json();
    const {
      id, category, address, institution_name, contact,
      credit_commission, private_commission, manager, memo, status,
    } = body;
    ...
    await logAction({ user_id: user.id, user_email: user.email, action: 'update', resource: '기관협약', resource_id: String(id), detail: `${current?.institution_name ?? `ID ${id}`} 수정`, meta: { changes } });
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    await logAction({ user_id: user.id, user_email: user.email, action: 'delete', resource: '기관협약', resource_id: ids.join(','), detail: `${ids.length}건 휴지통 이동`, meta: { ids } });
