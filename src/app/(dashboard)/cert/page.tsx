...
const CONSULTATION_STATUS_STYLE: Record<
  ConsultationStatus,
...
function getKstElapsedHours(dateStr: string): number {
  ...
  const kstCreated = new Date(
    created.toLocaleString("en-US", { timeZone: "Asia/Seoul" }),
  );
  ...
  if (
    ...
    return (
      <>
        ...
        <mark className={certStyles.highlightMark}>
...
function parseClickSource(source: string | null): {
  ...
  return {
    major,
    minor,
    needsCheck: isUnknownMamcafe || cleanedMinor === "확인필요",
  };
}
          ...
          {options.map((opt) => {
            ...
            return (
              ...
              >
                ...
                {value === opt && (
                  <span
                    className={certStyles.statusSelectCheck}
...
function StatCard({
  ...
  return (
    <div className={`${styles.statsCard} ${certStyles.statCardWrap}`}>
      <p className={`${styles.statsCardLabel} ${certStyles.statCardLabel}`}>
        {label}
      </p>
      <p
...
function DetailPanel({
  ...
  const [activeTab, setActiveTab] = useState<"basic" | "payment" | "memo">(
    "basic",
  );
  ...
  const [editBirthPrefix, setEditBirthPrefix] = useState(
    app.birth_prefix ?? "",
  );
  const [editAddress, setEditAddress] = useState(
    app.address_main ?? app.address ?? "",
  );
  ...
  const [editCashReceipt, setEditCashReceipt] = useState(
    app.cash_receipt ?? "",
  );
  ...
  return (
    ...
    >
      <div className={certStyles.detailModal}>
        {/* 헤더 */}
        <div className={certStyles.detailModalHeader}>
          <div className={certStyles.detailModalHeaderTop}>
            <div className={certStyles.detailModalHeaderLeft}>
              <div>
                <div className={certStyles.detailModalNameRow}>
                  <p className={certStyles.detailModalName}>{app.name}</p>
                  <PaymentBadge status={app.payment_status} />
                </div>
                <p className={certStyles.detailModalSub}>{app.contact}</p>
                <p className={certStyles.detailModalSub}>
            ...
            <button
              onClick={onClose}
              className={certStyles.detailModalCloseBtn}
            >
              ✕
            </button>
          </div>
        ...
        {/* 바디 */}
        <div className={certStyles.detailModalBody}>
          {activeTab === "basic" && (
            <>
              {/* 제출 사진 */}
                  ...
                  {(photoPreview || app.photo_url) && (
                    ...
                    >
                      ...
                      <img
                        src={
                          photoPreview ??
                          `${process.env.NEXT_PUBLIC_SUPABASE_URL}/storage/v1/object/public/photos/${app.photo_url}`
                        }
                        alt="제출 사진"
          ...
          {activeTab === "payment" && (
            <>
              ...
              <div className={styles.detailChipSection}>
                ...
                <div className={styles.detailChipRow}>
                  {(["pending", "paid", "failed", "cancelled"] as const).map(
                    (s) => {
              ...
              <div className={certStyles.detailInfoBox}>
                ...
                <DetailRow
                  ...
                  value={
                    app.paid_at
                      ? new Date(app.paid_at).toLocaleString("ko-KR")
                      : "-"
                  }
                />
                {app.failed_at && (
                ...
                {!app.cancelled_at && !app.failed_at && (
                  <DetailRow label="" value="" last />
                )}
              </div>
              ...
              <div className={certStyles.detailInfoBoxMt12}>
                <DetailRow
                  label="출처"
                  value={SOURCE_DISPLAY[app.source ?? ""] ?? app.source ?? "-"}
                />
                <DetailRow
                  label="신청일"
...
function DetailRow({
  ...
  return (
    ...
    >
      <span className={`${styles.infoRowLabel} ${certStyles.detailRowLabel}`}>
        {label}
      </span>
      <span
...
function AddCertModal({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (form.birth_prefix.trim())
        fd.append("birth_prefix", form.birth_prefix.trim());
      if (form.address.trim()) fd.append("address", form.address.trim());
      if (form.address_detail.trim())
        fd.append("address_detail", form.address_detail.trim());
      fd.append("certificates", JSON.stringify(selectedCerts));
      if (form.cash_receipt.trim())
        fd.append("cash_receipt", form.cash_receipt.trim());
      ...
      <div className={styles.funnelBox}>
        ...
        <div className={styles.funnelBody}>
            ...
            <div className={styles.funnelStep}>
              ...
              <div className={styles.funnelFieldGroup}>
                <label className={styles.funnelLabel}>
                  생년월일 <span className={styles.funnelOptional}>(선택)</span>
                </label>
                <input
                  value={form.birth_prefix}
                  onChange={(e) =>
                    setForm((p) => ({ ...p, birth_prefix: e.target.value }))
                  }
                  placeholder="예) 950101"
                  inputMode="numeric"
          ...
          {step === 2 &&
            (() => {
              const catOptions =
                CERT_CATEGORIES.find((c) => c.label === certCategory)
                  ?.options ?? [];
              const filteredOptions = certSearch.trim()
                ? CERT_CATEGORIES.flatMap((c) => c.options)
                    .filter((o, i, a) => a.indexOf(o) === i)
                    .filter((o) => o.includes(certSearch.trim()))
                : catOptions;
              return (
                <div className={styles.funnelStep}>
                  <p className={styles.funnelQuestion}>
                  ...
                  <p className={styles.funnelSubQuestion}>
                    모두 선택사항이에요
                  </p>
                  <div className={styles.funnelFieldGroup}>
                  ...
                  <div className={styles.funnelFieldGroup}>
                    <label className={styles.funnelLabel}>
                      ...
                      {selectedCerts.length > 0 && (
                        <span className={certStyles.selectedCertCount}>
                          {selectedCerts.length}개 선택
                        </span>
                      )}
                    </label>
                    ...
                    {selectedCerts.length > 0 && (
                      <div className={certStyles.selectedCertTagRow}>
                        {selectedCerts.map((cert) => (
                          <span
                            key={cert}
                            className={certStyles.selectedCertTag}
                          >
                            {cert}
                            <button
                              type="button"
                        ...
                        filteredOptions.map((opt) => {
                          ...
                          return (
                            <button
                              ...
                              className={
                                isSelected
                                  ? certStyles.certListItemSelected
                                  : certStyles.certListItem
                              }
                            >
                              <span
                                className={
                                  isSelected
                                    ? certStyles.certListCheckboxSelected
                                    : certStyles.certListCheckbox
                                }
                              >
                                {isSelected && "✓"}
                              </span>
            ...
            <div className={styles.funnelStep}>
              ...
              <div className={styles.funnelFieldGroup}>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setForm((p) => ({ ...p, cash_receipt: e.target.value }))
                  }
                  placeholder="현금영수증 번호"
                  className={styles.funnelInput}
                />
              ...
              <div className={styles.funnelFieldGroup}>
                <label className={styles.funnelLabel}>
                  사진 <span className={styles.funnelOptional}>(선택)</span>
                </label>
                <input
                  ref={fileInputRef}
                  type="file"
...
function PCertDetailPanel({
  item,
  onClose,
  onUpdate,
  initialTab = "basic",
}: PCertDetailPanelProps) {
  ...
  const [editClickSource, setEditClickSource] = useState(
    item.click_source ?? "",
  );
  ...
  const [saving, setSaving] = useState(false);
  const [activeTab, setActiveTab] = useState<"basic" | "info" | "memo">(
    initialTab,
  );
  const [memoCount, setMemoCount] = useState<number | null>(null);
        ...
        <div className={certStyles.detailModalHeader}>
          <div className={certStyles.detailModalHeaderTop}>
            <div className={certStyles.detailModalHeaderLeft}>
              <div>
                <div className={certStyles.detailModalNameRow}>
                  <p className={certStyles.detailModalName}>{item.name}</p>
                  <StatusBadge
                    status={editStatus}
                    styleMap={CONSULTATION_STATUS_STYLE}
                  />
                </div>
                <p className={certStyles.detailModalSub}>{item.contact}</p>
                <p className={certStyles.detailModalSub}>
              ...
              <div className={styles.detailChipSection}>
                ...
                {editCounselCheck.includes("기타") && (
                  <input
                    ...
                    className={`${styles.input} ${styles.inputFull} ${certStyles.mt8}`}
                    autoFocus
                  />
                )}
              </div>
                      ...
                      style={
                        editStatus === s
                          ? {
                              ...
                              background:
                                CONSULTATION_STATUS_STYLE[s].background,
        ...
        <div className={styles.funnelBody}>
            ...
            <div className={styles.funnelStep}>
              ...
              <div className={styles.funnelFieldGroup}>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setForm((p) => ({ ...p, major_category: e.target.value }))
                  }
              ...
              <div className={styles.funnelFieldGroup}>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setForm((p) => ({ ...p, click_source: e.target.value }))
                  }
          ...
          {step === 3 && (
            <div className={styles.funnelStep}>
              ...
              <div className={styles.funnelFieldGroup}>
                <label className={styles.funnelLabel}>
                  메모 <span className={styles.funnelOptional}>(선택)</span>
                </label>
                <textarea
                  value={form.memo}
                  onChange={(e) =>
...
function PCertHighlight({
  ...
  if (hasDirectMatch) {
    return (
      <>
        {parts.map((part, i) =>
          part.toLowerCase() === query.toLowerCase() ? (
            <mark key={i} className={certStyles.pcertHighlightMark}>
              {part}
            </mark>
          ) : (
            part
  ...
  if (
    ...
    return (
      <>
        {text.slice(0, origStart)}
        <mark className={certStyles.pcertHighlightMark}>
          {text.slice(origStart, origEnd)}
        </mark>
        {text.slice(origEnd)}
      </>
...
function PrivateCertTab({ highlightId }: { highlightId?: number }) {
  ...
  useEffect(() => {
    ...
    const mgrs = Array.from(
      new Set(statsItems.map((c) => c.manager).filter(Boolean)),
    ) as string[];
    if (mgrs.length === 0) {
    ...
    const rate = (list: PrivateCert[]) => {
      ...
      return t > 0
        ? Math.round(
            (list.filter((c) => c.status === "등록완료").length / t) * 100,
          )
        : 0;
    };
    const mStats = mgrs
  ...
  const uniqueMajorCategories = Array.from(
    new Set(
      items.map((c) => parseClickSource(c.click_source).major).filter(Boolean),
    ),
  ).sort();
  const needsCheckCount = items.filter(
    (c) => parseClickSource(c.click_source).needsCheck,
  ).length;
      ...
      items
        .filter(
          (c) =>
            ...
            parseClickSource(c.click_source).major === majorCategoryFilter,
        )
  ...
  const isFiltered =
    ...
    managerFilter !== "all" ||
    majorCategoryFilter !== "all" ||
    minorCategoryFilter !== "all" ||
    reasonFilter !== "all" ||
    counselCheckFilter !== "all" ||
    startDate ||
    endDate;
  ...
  const handlePCertDownloadSelected = () => {
    ...
    downloadExcel(
      `민간자격증_선택_${new Date().toLocaleDateString("ko-KR").replace(/\. /g, "-").replace(".", "")}.xlsx`,
      [
        {
          name: "민간자격증",
  ...
  const handlePCertDownloadAll = () => {
    downloadExcel(
      `민간자격증_전체_${new Date().toLocaleDateString("ko-KR").replace(/\. /g, "-").replace(".", "")}.xlsx`,
      [
        {
          name: "민간자격증",
          headers: PCERT_HEADERS,
        ...
        <>
          <div className={styles.filterRow}>
            ...
            <input
              ...
              className={`${styles.input} ${certStyles.dateInput140}`}
            />
            ...
            {selectedIds.length > 0 && (
              <>
                ...
                <button
                  onClick={handlePCertDownloadSelected}
          ...
          <div className={styles.actionBar}>
            ...
            <button
              onClick={handlePCertDownloadAll}
              className={styles.btnDownload}
            >
              ↓ 전체 다운로드
            </button>
            ...
            <table className={styles.table}>
              ...
              <tbody>
                {loading ? (
                  ...
                  <tr>
                    <td colSpan={13} className={styles.tableEmptyMsg}>
                      검색 결과가 없습니다.
                    </td>
                  </tr>
                ) : (
                  paginated.map((item, index) => (
                    ...
                    >
                      ...
                      <td
                        className={`${styles.tdSecondary}${parseClickSource(item.click_source).needsCheck ? ` ${certStyles.tdNeedsCheck}` : ""}`}
                      >
                        {parseClickSource(item.click_source).minor || "-"}
                      </td>
                      <td className={styles.tdBold}>
                        <PCertHighlight text={item.name} query={searchText} />
                      </td>
                      <td className={styles.tdTabular}>
                        <PCertHighlight
                          text={item.contact}
                          query={searchText}
                        />
                      ...
                      <td className={styles.tdEllipsis}>
                        {item.hope_course ?? "-"}
                      </td>
                      <td
                        ...
                        title={item.reason ?? ""}
                      >
                        <PCertHighlight text={item.reason} query={searchText} />
                      </td>
                      <td className={styles.tdSecondary}>
                        {item.manager ?? "-"}
                      </td>
                        ...
                        {item.counsel_check ? (
                          <div className={styles.counselChipRow}>
                            {item.counsel_check
                              ...
                              .filter(Boolean)
                              .map((c) => (
                                <span key={c} className={styles.counselChip}>
                                  {c}
                                </span>
                              ))}
                          </div>
                      ...
                      >
                        <MemoHoverBadge
                          text={item.latest_memo || item.memo || "-"}
                        />
                      </td>
      ...
      {totalPages > 1 && (
        <div className={styles.pagination}>
          <button
            ...
            className={`${styles.pageBtn} ${certStyles.pageBtnMr4}`}
          >
            ‹
          </button>
          ...
          <button
            ...
            className={`${styles.pageBtn} ${certStyles.pageBtnMl4}`}
          >
            ›
          </button>
        </div>
      ...
      {openFilterColumn && (
        ...
        >
          {openFilterColumn === "major" && (
            <>
              <div
                className={`${styles.filterDropdownItem}${majorCategoryFilter === "all" ? ` ${styles.filterDropdownItemActive}` : ""}`}
                onClick={() => {
                  setMajorCategoryFilter("all");
                  setMinorCategoryFilter("all");
                  setCurrentPage(1);
              ...
              {uniqueMajorCategories.map((m) => (
                <div
                  key={m}
                  className={`${styles.filterDropdownItem}${majorCategoryFilter === m ? ` ${styles.filterDropdownItemActive}` : ""}`}
                  onClick={() => {
                    setMajorCategoryFilter(m);
                    setMinorCategoryFilter("all");
                    setCurrentPage(1);
                    setOpenFilterColumn(null);
                  }}
          ...
          {openFilterColumn === "minor" && (
            <>
              <div
                className={`${styles.filterDropdownItem}${minorCategoryFilter === "all" ? ` ${styles.filterDropdownItemActive}` : ""}`}
                onClick={() => {
                  setMinorCategoryFilter("all");
                  setCurrentPage(1);
                  setOpenFilterColumn(null);
              ...
              {needsCheckCount > 0 && (
                <div
                  className={`${styles.filterDropdownItem}${minorCategoryFilter === "__needs_check__" ? ` ${styles.filterDropdownItemActive}` : ""} ${certStyles.filterNeedsCheck}`}
                  onClick={() => {
                    setMinorCategoryFilter("__needs_check__");
                    setCurrentPage(1);
                    setOpenFilterColumn(null);
                  }}
                >
              ...
              {uniqueMinorCategories.map((m) => (
                <div
                  key={m}
                  className={`${styles.filterDropdownItem}${minorCategoryFilter === m ? ` ${styles.filterDropdownItemActive}` : ""}`}
                  onClick={() => {
                    setMinorCategoryFilter(m);
                    setCurrentPage(1);
                    setOpenFilterColumn(null);
                  }}
                >
          ...
          {openFilterColumn === "reason" && (
            <>
              <div
                className={`${styles.filterDropdownItem}${reasonFilter === "all" ? ` ${styles.filterDropdownItemActive}` : ""}`}
                onClick={() => {
                  setReasonFilter("all");
                  setCurrentPage(1);
                  setOpenFilterColumn(null);
              ...
              {REASON_OPTIONS.map((r) => (
                <div
                  key={r}
                  className={`${styles.filterDropdownItem}${reasonFilter === r ? ` ${styles.filterDropdownItemActive}` : ""}`}
                  onClick={() => {
                    setReasonFilter(r);
                    setCurrentPage(1);
                    setOpenFilterColumn(null);
          ...
          {openFilterColumn === "counsel" && (
            <>
              <div
                className={`${styles.filterDropdownItem}${counselCheckFilter === "all" ? ` ${styles.filterDropdownItemActive}` : ""}`}
                onClick={() => {
                  setCounselCheckFilter("all");
                  setCurrentPage(1);
                  setOpenFilterColumn(null);
              ...
              {COUNSEL_CHECK_OPTIONS.map((c) => (
                <div
                  key={c}
                  className={`${styles.filterDropdownItem}${counselCheckFilter === c ? ` ${styles.filterDropdownItemActive}` : ""}`}
                  onClick={() => {
                    setCounselCheckFilter(c);
                    setCurrentPage(1);
                    setOpenFilterColumn(null);
                  }}
                >
            ...
            <>
              ...
              {CONSULTATION_STATUS_OPTIONS.map((s) => (
                <div
                  ...
                  onClick={() => {
                    setStatusFilter(s as ConsultationStatus);
                    setCurrentPage(1);
                    setOpenFilterColumn(null);
                  }}
                >
...
function ApplicationTab({
  ...
  const sourceOptions = [
    ...
    ...Array.from(new Set(applications.map((a) => a.source).filter(Boolean)))
      ...
      .map((s) => ({
        value: `src:${s}`,
        label: SOURCE_DISPLAY[s as string] ?? (s as string),
      })),
  ...
  const isIndeterminate =
    paginated.some((a) => selectedIds.has(a.id)) && !isAllSelected;
  ...
  const handleAppDownloadSelected = () => {
    ...
    downloadExcel(
      `${tabLabel}_선택_${new Date().toLocaleDateString("ko-KR").replace(/\. /g, "-").replace(".", "")}.xlsx`,
      [
        {
          name: tabLabel,
          headers: APP_HEADERS,
  ...
  const handleAppDownloadAll = () => {
    downloadExcel(
      `${tabLabel}_전체_${new Date().toLocaleDateString("ko-KR").replace(/\. /g, "-").replace(".", "")}.xlsx`,
      [
        {
          name: tabLabel,
          headers: APP_HEADERS,
  ...
  return (
    <div className={certStyles.appTabWrap}>
          ...
          <div className={styles.filterRow}>
            ...
            {selectedIds.size > 0 && (
              <>
                ...
                <button
                  onClick={handleAppDownloadSelected}
                  className={styles.btnDownload}
                >
                  ↓ 선택 다운로드
                </button>
          ...
          <table className={`${styles.table} ${certStyles.tableMinW900}`}>
            <thead>
              <tr>
                {/* 전체 선택 체크박스 */}
                <th
                  className={`${styles.thCenter} ${certStyles.thCheckboxW44}`}
                >
                  <input
                    type="checkbox"
                    className={styles.checkbox}
                ...
                <th className={styles.thFilterable}>
                  <div className={styles.thInner}>
                    ...
                    <button
                      ...
                      onClick={(e) => {
                        e.stopPropagation();
                        if (openFilterColumn === "payment_status") {
                          setOpenFilterColumn(null);
                          return;
                        }
                        const rect = e.currentTarget.getBoundingClientRect();
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        ...
                        {app.is_checked && (
                          <span
                            className={`${styles.statusBadge} ${certStyles.issuedBadge}`}
                          >
                            발급완료
                          </span>
                        )}
                      ...
                      >
                        ...
                        {app.ref && (
                          <span
                            className={`${styles.statusBadge} ${certStyles.refBadge}`}
                          >
                            {app.ref}
                          </span>
                        )}
          ...
          {paginationPages.map((page, idx) =>
            page === "..." ? (
              ...
              <button
                ...
                onClick={() => setCurrentPage(page as number)}
                className={
                  safePage === page ? styles.pageBtnActive : styles.pageBtn
                }
              >
                {page}
              </button>
      ...
      {deleteToastVisible2 && (
        <div className={styles.toast}>삭제되었습니다</div>
      )}
...
function certYm(d: Date): string {
  return d.toISOString().slice(0, 7);
}
function certYmd(d: Date): string {
...
function CertStudentDetailPanel({
  item,
  onClose,
  onUpdate,
  initialTab = "basic",
  ...
  const [editCourseItems, setEditCourseItems] = useState<CourseItem[]>(() =>
    parseCourseItems(item.course),
  );
  ...
  const [memoCount, setMemoCount] = useState<number | null>(null);
  const [localMemoCount, setLocalMemoCount] = useState<number>(
    item.memo_count ?? 0,
  );
  const [localLastMemoAt, setLocalLastMemoAt] = useState<string | null>(
    item.latest_memo_at ?? null,
  );
      ...
      await onUpdate(item.id, {
        ...
        course:
          editCourseItems.length > 0
            ? serializeCourseItems(editCourseItems)
            : null,
            ...
            <div className={certStyles.detailModalHeaderLeft}>
              <div>
                ...
                <div className={styles.detailModalSubRow}>
                  <span className={certStyles.detailModalSub}>
                    등록일: {formatDateShort(item.created_at)}
                  </span>
                  <DateInput
                    value={
                ...
                <div className={certStyles.detailModalContactRow}>
                  <span className={certStyles.detailModalContactBadge}>
                    총 {localMemoCount}회 연락
                  </span>
                  {localLastMemoAt && (
                    <span className={certStyles.detailModalContactSub}>
          ...
          {activeTab === "basic" && (
            <>
                  ...
                  {STUDENT_STATUS_OPTIONS.map((s) => (
                    <button
                      ...
                      style={
                        editStatus === s
                          ? {
                              border: `2px solid ${STUDENT_STATUS_STYLE[s].color}`,
                              background: STUDENT_STATUS_STYLE[s].background,
                              color: STUDENT_STATUS_STYLE[s].color,
                            }
                          : undefined
                      }
                    >
              ...
              <div className={styles.detailFieldRow}>
                ...
                <CourseTagInput
                  items={editCourseItems}
                  onChange={setEditCourseItems}
                  placeholder="과목 검색 후 추가..."
                />
              </div>
              <div className={styles.detailFieldRow}>
    ...
    try {
      const res = await fetch("/api/cert/students", {
        ...
        body: JSON.stringify({
          ...form,
          course:
            form.courseItems.length > 0
              ? serializeCourseItems(form.courseItems)
              : null,
          completion_rate: null,
        }),
      });
        ...
        <div className={styles.funnelFooter}>
          {step < TOTAL_STEPS ? (
            <button
              type="button"
              onClick={handleNext}
              className={styles.funnelNextBtn}
            >
              다음
            </button>
          ) : (
            <button
              ...
              onClick={handleSubmit}
              disabled={saving}
              className={`${styles.funnelNextBtn}${saving ? ` ${styles.funnelNextBtnDisabled}` : ""}`}
            >
              {saving ? "저장 중..." : "등록하기"}
            </button>
          )}
...
function StudentMgmtTab() {
  ...
  const uniqueSources = Array.from(
    new Set(items.map((c) => c.click_source).filter(Boolean)),
  ).sort() as string[];
  ...
  const filtered = items.filter((c) => {
    ...
    if (
      courseFilter &&
      (c.course || "").toLowerCase() !== courseFilter.toLowerCase()
    )
      return false;
    if (statusFilter !== "all" && c.status !== statusFilter) return false;
    if (managerFilter === "none" && c.manager) return false;
    if (
      managerFilter !== "all" &&
      managerFilter !== "none" &&
      c.manager !== managerFilter
    )
      return false;
    if (sourceFilter.length > 0 && !sourceFilter.includes(c.click_source ?? ""))
      return false;
    if (startDate || endDate) {
      const d = new Date(c.created_at);
      if (startDate && d < new Date(startDate + "T00:00:00")) return false;
  ...
  const handleDownloadAll = () => {
    downloadExcel(
      `학생관리_전체_${new Date().toLocaleDateString("ko-KR").replace(/\. /g, "-").replace(".", "")}.xlsx`,
      [
        {
          name: "학생관리",
  ...
  const handleDownloadSelected = () => {
    const targets = filtered.filter((c) => selectedIds.includes(c.id));
    downloadExcel(
      `학생관리_선택_${new Date().toLocaleDateString("ko-KR").replace(/\. /g, "-").replace(".", "")}.xlsx`,
      [
        {
          name: "학생관리",
          headers: STUDENT_HEADERS,
                ...
                <tr>
                  ...
                  <th className={styles.thFilterable}>
                    <div className={styles.thInner}>
                      유입경로
                      <button
                        className={`${styles.thFilterBtn}${sourceFilter.length > 0 ? ` ${styles.thFilterBtnActive}` : ""}`}
                        onClick={(e) => {
                          e.stopPropagation();
                          if (openFilterColumn === "source") {
                            setOpenFilterColumn(null);
                  ...
                  paginated.map((item, index) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          {(() => {
                            const s = parseClickSource(item.click_source);
                            return s.minor
                              ? `${s.major} > ${s.minor}`
                              : s.major || "-";
                          })()}
                          ...
                          {courseItems.length > 0 ? (
                            <div className={certStyles.courseTableCellList}>
                              {courseItems.map((c) => (
                                <span
                                  key={c.name}
                                  className={certStyles.courseTableTag}
                                >
                                  {c.name} {c.rate ?? 0}%
                                </span>
                              ))}
                            ...
                            <div className={certStyles.studentRateCellStack}>
                              {courseItems.map((c) => {
                                ...
                                return (
                                  <div
                                    key={c.name}
                                    className={certStyles.studentRateCellRow}
                                  >
                                    <div
                                      className={certStyles.studentRateCellBar}
                                    >
                                      <div
                                        className={
                                          certStyles.studentRateCellBarFill
                                        }
                                        style={{
                                    ...
                                    <span
                                      className={certStyles.studentRateCellText}
                                    >
                                      {r}%
                                    </span>
                                  </div>
                        ...
                        >
                          <StatusSelect
                            value={item.status}
                            onChange={(v) =>
                              handleStatusChange(item.id, v as StudentStatus)
                            }
      ...
      {openFilterColumn && (
        ...
        >
          {openFilterColumn === "source" && (
            <>
              <div
                className={`${styles.filterDropdownItem}${sourceFilter.length === 0 ? ` ${styles.filterDropdownItemActive}` : ""}`}
                onClick={() => {
                  setSourceFilter([]);
                  setCurrentPage(1);
                  setOpenFilterColumn(null);
              ...
              {uniqueSources.map((s) => (
                <div
                  key={s}
                  className={`${styles.filterDropdownItem}${sourceFilter.includes(s) ? ` ${styles.filterDropdownItemActive}` : ""}`}
                  onClick={() => {
...
function StudentContactTab() {
  ...
  const uniqueClickSources = Array.from(
    new Set(eligible.map((c) => c.click_source).filter(Boolean)),
  ).sort() as string[];
  ...
  const filtered = eligible.filter((c) => {
    ...
    if (
      clickSourceFilter.length > 0 &&
      !clickSourceFilter.includes(c.click_source ?? "")
    )
      return false;
    return true;
  });
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [searchText, statusFilter, managerFilter, clickSourceFilter]);
        ...
        <table className={styles.table}>
            ...
            <tr>
              ...
              <th className={styles.thFilterable}>
                <div className={styles.thInner}>
                  유입경로
                  <button
                    className={`${styles.thFilterBtn}${clickSourceFilter.length > 0 ? ` ${styles.thFilterBtnActive}` : ""}`}
                    onClick={(e) => {
                      e.stopPropagation();
                      if (openFilterColumn === "source") {
                        setOpenFilterColumn(null);
          ...
          <tbody>
            {loading ? (
              ...
              <tr>
                <td colSpan={11} className={styles.tableEmptyMsg}>
                  연락예정 항목이 없습니다.
                </td>
              </tr>
            ) : (
              paginated.map((item, idx) => (
                <tr
                  ...
                  style={{
                    cursor: "pointer",
                    background:
                      selectedItem?.id === item.id ? "#EBF3FE" : "white",
                  }}
                  onMouseEnter={(e) => {
      ...
      {openFilterColumn && (
        ...
        >
          {openFilterColumn === "source" && (
            <>
              <div
                className={`${styles.filterDropdownItem}${clickSourceFilter.length === 0 ? ` ${styles.filterDropdownItemActive}` : ""}`}
                onClick={() => {
                  setClickSourceFilter([]);
                  setCurrentPage(1);
                  setOpenFilterColumn(null);
              ...
              {uniqueClickSources.map((s) => (
                <div
                  key={s}
                  className={`${styles.filterDropdownItem}${clickSourceFilter.includes(s) ? ` ${styles.filterDropdownItemActive}` : ""}`}
                  onClick={() => {
                    setClickSourceFilter((prev) =>
                      prev.includes(s)
                        ? prev.filter((x) => x !== s)
                        : [...prev, s],
                    );
...
function StudentBulkUploadView({ onBack }: { onBack: () => void }) {
  ...
  async function handleConfirm() {
    ...
    if (
      !confirm(
        `선택한 ${selectedDraftIds.length}건을 학생관리로 이동하시겠습니까?`,
      )
    )
      return;
    setConfirming(true);
  ...
  const PREVIEW_COLS: (keyof StudentCsvRow)[] = [
    "name",
    "contact",
    "course",
    "status",
    ...
    <div className={styles.bulkWrap}>
      ...
      <div className={certStyles.draftSection}>
        <div className={certStyles.draftSectionHeader}>
          ...
          {drafts.length > 0 && (
            <span className={certStyles.draftSectionCount}>
              {drafts.length}건
            </span>
          )}
          ...
          {selectedDraftIds.length > 0 && (
            ...
            >
              {confirming
                ? "이동 중..."
                : `선택한 ${selectedDraftIds.length}건 학생관리로 이동`}
            </button>
          )}
        </div>
        ...
        {draftsLoading ? (
          ...
          <div className={certStyles.draftEmpty}>
            임시저장된 항목이 없습니다. CSV를 업로드해서 임시저장해보세요.
          </div>
        ) : (
          <div className={styles.tableCard}>
            <div className={styles.tableOverflow}>
          ...
          <div className={styles.bulkUploadArea}>
            ...
            <div className={styles.bulkGuideBox}>
              ...
              <div className={certStyles.bulkCourseGuide}>
                <p className={certStyles.bulkCourseGuideTitle}>
                  📌 수강과목 입력 방법
                </p>
                <div className={certStyles.bulkCourseGuideRow}>
                  <span className={certStyles.bulkCourseGuideLabel}>
                    단일 과목
                  </span>
                  <code className={certStyles.bulkCourseGuideCode}>
                    생활지원사1급:80
                  </code>
                  <span className={certStyles.bulkCourseGuideHint}>
                    과목명:수강률(0~100)
                  </span>
                </div>
                <div className={certStyles.bulkCourseGuideRow}>
...
type TemplateSubTab =
  ...
  | "과정안내"
  | "수강안내";
const TEMPLATE_SUB_TABS: TemplateSubTab[] = [
  "첫 안내",
  "수업",
  "시험",
  "취업",
...
function DownloadAllImagesBtn({
  ...
  return (
    <button
      type="button"
      className={certStyles.templateImageSaveAllBtn}
      onClick={handleDownload}
      disabled={state === "loading"}
    >
      {state === "loading"
        ? "ZIP 만드는 중..."
        : `이미지 ZIP 저장${urls.length > 1 ? ` (${urls.length}개)` : ""}`}
    </button>
  );
}
...
function CounselTemplateTab() {
  ...
  useEffect(() => {
    fetch("/api/auth/me")
      ...
      .then((data) => {
        ...
        const isExec = ["이사", "상무"].some((t) =>
          data.displayName?.includes(t),
        );
  ...
  const fetchBlocks = async () => {
    ...
    const res = await fetch(
      `/api/cert/templates?sub_tab=${encodeURIComponent(activeTab)}`,
    );
  ...
  const handleDelete = async (block: TemplateBlock) => {
    ...
    const imagePaths = block.images
      .map((url) => url.split("/cert-templates/")[1])
      .filter(Boolean);
      ...
      <div className={certStyles.templateBody}>
        {loading ? (
          ...
          <>
              ...
              <div key={block.id} className={certStyles.templateBlock}>
                <div className={certStyles.templateBlockTitleRow}>
                  ...
                  {isAdmin && (
                    <div className={certStyles.templateBlockActions}>
                      ...
                      <button
                        type="button"
                        className={certStyles.templateDeleteBtn}
                ...
                {block.content && (
                  <div className={certStyles.templateBlockContent}>
                    <pre className={certStyles.templateBlockText}>
                      {block.content}
                    </pre>
                    <button
                      type="button"
                    ...
                    >
                      {copiedId === block.id ? (
                        ...
                        >
                          ...
                          />
                          <path
                            d="M2 10V2.5A.5.5 0 0 1 2.5 2H10"
                            stroke="currentColor"
                            strokeWidth="1.4"
                            strokeLinecap="round"
                          />
                    ...
                    <div className={certStyles.templateImageRow}>
                      {block.images.map((url, i) => (
                        <img
                          ...
                          src={url}
                          alt=""
                          className={certStyles.templateImage}
                        />
                      ))}
                    </div>
            ...
            {blocks.length === 0 && (
              <p className={certStyles.templateEmpty}>
                등록된 템플릿이 없습니다.
              </p>
            )}
            {isAdmin && (
              <TemplateAddModal subTab={activeTab} onSaved={fetchBlocks} />
            )}
          </>
        )}
      </div>
...
function TemplateAddModal({
  ...
  if (!open)
    return (
      <button
        type="button"
        className={certStyles.templateAddBtn}
  ...
  return (
    <div className={certStyles.templateForm}>
      ...
      <textarea
        ...
        rows={4}
        className={certStyles.templateFormTextarea}
      />
      <div className={certStyles.templateImageUploadRow}>
        {images.map((img, i) => (
          <div key={i} className={certStyles.templateImageThumb}>
            ...
            <button
              type="button"
              className={certStyles.templateImageRemove}
        ...
        <button
          type="button"
          className={certStyles.templateImageUploadBtn}
...
function TemplateEditModal({
  ...
  const [images, setImages] = useState<{ path: string; url: string }[]>(
    block.images.map((url) => ({
      path: url.split("/cert-templates/")[1] ?? "",
      url,
    })),
  );
  const [uploading, setUploading] = useState(false);
  ...
  if (!open)
    return (
      <button
        type="button"
        className={certStyles.templateEditBlockBtn}
        onClick={() => setOpen(true)}
      >
        수정
      </button>
  ...
  return (
    <div
      className={certStyles.templateFormOverlay}
      onClick={(e) => {
...
function CertStatsTab() {
    ...
    const fetchData = async () => {
      ...
      if (source === "private-cert" || source === "all") {
        ...
        if (Array.isArray(pcData)) {
          pcData.forEach(
            (c: { id: number; status: string; created_at: string }) => {
              items.push({
                id: c.id,
                status: c.status,
                created_at: c.created_at,
  ...
  // cert 상태 집계
  const certStatusData = ["paid", "pending", "failed", "cancelled"]
    .map((s) => ({
  ...
  // 민간자격증 상태 집계
  const pcertStatusData = (CONSULTATION_STATUS_OPTIONS as string[])
    .map((s) => ({
      name: s,
  ...
  const statusData = isCertOnly
    ? certStatusData
    : isPcertOnly
      ? pcertStatusData
      : [...certStatusData, ...pcertStatusData];
  ...
  const weekData = WEEKDAY_KO.map((day, i) => ({
    day,
    count: data.filter((c) => certToKST(c.created_at).getDay() === i).length,
  }));
  const maxWeekCount =
...
export default function CertPage() {
    ...
    fetch("/api/auth/me")
      ...
      .then((data) => {
        if (data) {
          ...
          if (!isFullAccess) {
            const certPerm = data.permissions?.find(
            ...
            const normalized = raw
              ? raw.map((v) =>
                  v.startsWith("cert-tab-") ? v.slice("cert-tab-".length) : v,
                )
              : null;
  ...
  const MANAGED_CERT_TAB_KEYS = new Set([
    "hakjeom",
    "edu",
    "private-cert",
    "student-mgmt",
  ...
  const visibleTabs = allowedCertTabs
    ? SOURCE_TABS.filter(
        (t) =>
          allowedCertTabs.includes(t.value) ||
          !MANAGED_CERT_TAB_KEYS.has(t.value),
      )
    : SOURCE_TABS;
  ...
  useEffect(() => {
    ...
    if (
      urlTab &&
      SOURCE_TABS.some((t) => t.value === urlTab) &&
      (!allowed || allowed.includes(urlTab))
    ) {
      setSourceTab(urlTab);
  ...
  return (
    <div>
      {/* 탭 컨텐츠 */}
      {(sourceTab === "hakjeom" || sourceTab === "edu") && (
        <ApplicationTab
          sourceTab={sourceTab}
          highlightId={urlTab === sourceTab ? urlHighlight : undefined}
        />
      )}
      {sourceTab === "private-cert" && (
        <PrivateCertTab
          highlightId={urlTab === "private-cert" ? urlHighlight : undefined}
        />
      )}
      {sourceTab === "student-mgmt" && <StudentMgmtTab />}
      {sourceTab === "student-contact" && <StudentContactTab />}
      {sourceTab === "student-bulk" && (
        <StudentBulkUploadView onBack={() => setSourceTab("student-mgmt")} />
      )}
