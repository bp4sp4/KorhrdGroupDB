...
function MonthPicker({
  ...
  return (
    <div className={styles.month_picker_wrap} ref={wrapRef}>
      <button
        className={styles.month_picker_btn}
        onClick={() => {
          setPickerYear(year);
          setOpen((v) => !v);
        }}
      ...
      {open && (
        <div className={styles.month_picker_dropdown}>
          <div className={styles.month_picker_year_row}>
            <button
              className={styles.month_picker_year_btn}
              onClick={() => setPickerYear((y) => y - 1)}
            >
              <ChevronLeft size={14} />
            </button>
            <span className={styles.month_picker_year_label}>
              {pickerYear}년
            </span>
            <button
              className={styles.month_picker_year_btn}
...
function NmsTab({
  ...
  const [allcareTypeFilter, setAllcareTypeFilter] = useState<Set<string>>(
    new Set(),
  );
    ...
    <div className={styles.tab_content}>
      <div className={styles.filter_panel}>
        <div className={styles.team_tabs}>
          <button
            className={`${styles.team_tab} ${teamFilter === "" ? styles.team_tab_active : ""}`}
          ...
          {NMS_TEAMS.map((t) => (
            <button
              key={t}
              className={`${styles.team_tab} ${teamFilter === t ? styles.team_tab_active : ""}`}
          ...
          <button
            className={`${styles.team_tab} ${teamFilter === "allcare" ? styles.team_tab_active : ""}`}
            onClick={() => setTeamFilter("allcare")}
          >
            올케어
          </button>
          <button
            className={`${styles.team_tab} ${teamFilter === "edu" ? styles.team_tab_active : ""}`}
            onClick={() => setTeamFilter("edu")}
          >
            교육원
          </button>
        </div>
        ...
        <div className={styles.summary_grid}>
          <div className={`${styles.summary_card} ${styles.card_purple}`}>
            ...
            <span className={styles.summary_value}>
              {formatAmount(eduData?.totalRevenue ?? 0)}
            </span>
            <span className={styles.summary_sub}>
              {eduData?.count ?? 0}명 등록 (환불/삭제 제외)
            </span>
          </div>
        </div>
      )}
      ...
      {data && (
        <div className={styles.summary_grid}>
            ...
            <span className={styles.summary_value}>
              {formatAmount(
                data.total.paymentAmount +
                  (allcareData?.totalRevenue ?? 0) +
                  (eduData?.totalRevenue ?? 0),
              )}
            </span>
            <span className={styles.summary_sub}>
              학점은행제 + 올케어 + 교육원 합산
          ...
          <div className={`${styles.summary_card} ${styles.card_blue}`}>
            ...
            <span className={styles.summary_value}>
              {formatAmount(data.total.paymentAmount)}
            </span>
          ...
          <div className={`${styles.summary_card} ${styles.card_green}`}>
            ...
            <span className={styles.summary_value}>
              {formatAmount(allcareData?.totalRevenue ?? 0)}
            </span>
            <span className={styles.summary_sub}>
              {allcareData?.count ?? 0}건 결제완료
            </span>
          </div>
          <div className={`${styles.summary_card} ${styles.card_purple}`}>
            <div className={styles.card_icon_wrap}>
      ...
      {loading ? (
        ...
        <div className={styles.allcare_wrap}>
          {(() => {
            ...
            const allPayments = (allcareData?.payments ?? []).filter(
              (p) => isAll || allcareTypeFilter.has(p.type),
            );
            const totalPages = Math.ceil(
              allPayments.length / ALLCARE_PAGE_SIZE,
            );
            const paginated = allPayments.slice(
              (allcarePage - 1) * ALLCARE_PAGE_SIZE,
              allcarePage * ALLCARE_PAGE_SIZE,
            );
            return (
              <>
                <div className={styles.allcare_table_card}>
                  <table className={styles.allcare_table}>
                      ...
                      <tr>
                        <th className={styles.allcare_thFilterable}>
                          ...
                          {typeDropdownOpen &&
                            (() => {
                              const rect =
                                typeFilterBtnRef.current?.getBoundingClientRect();
                              return (
                                <div
                                  ref={typeDropdownRef}
                                ...
                                >
                                  <div
                                    className={styles.allcare_checkItem}
                                    onClick={toggleAll}
                                  >
                                    <span
                                      className={`${styles.allcare_checkbox}${isAll ? ` ${styles.allcare_checkboxChecked}` : ""}`}
                                    >
                                      {isAll && (
                                        <svg
                                          width="10"
                                  ...
                                  {typeKeys.map((type) => {
                                    ...
                                    return (
                                      ...
                                      >
                                        <span
                                          className={`${styles.allcare_checkbox}${checked ? ` ${styles.allcare_checkboxChecked}` : ""}`}
                                        >
                                          {checked && (
                                            <svg
                                              width="10"
                        ...
                        <th
                          className={`${styles.allcare_th} ${styles.th_right}`}
                        >
                          금액
                        </th>
                        <th
                          className={`${styles.allcare_th} ${styles.th_center}`}
                        >
                          결제수단
                        </th>
                        <th
                    ...
                    <tbody>
                      {paginated.length === 0 ? (
                        <tr>
                          <td colSpan={7} className={styles.allcare_empty_td}>
                            데이터 없음
                          </td>
                        </tr>
                      ) : (
                        paginated.map((p) => (
                          <tr key={p.id} className={styles.allcare_tr}>
                            <td className={styles.allcare_td}>
                              <span
                                className={`${styles.allcare_type_chip} ${styles[`chip_${p.type}`]}`}
                              >
                                {p.type}
                              </span>
                            </td>
                            <td
                              className={`${styles.allcare_td} ${styles.manager_name}`}
                            >
                              {p.user_name ?? "-"}
                            </td>
                            <td className={styles.allcare_td}>
                            ...
                            <td
                              className={`${styles.allcare_td} ${styles.td_right}`}
                            >
                              {formatAmount(p.amount)}
                            </td>
                            <td
                              className={`${styles.allcare_td} ${styles.td_center}`}
                            >
                              {p.payment_method ?? "-"}
                            </td>
                            <td
                ...
                {totalPages > 1 && (
                  <div className={styles.allcare_pagination}>
                    <button
                      ...
                      disabled={allcarePage === 1}
                      onClick={() => setAllcarePage((p) => p - 1)}
                    >
                      <ChevronLeft size={14} />
                    </button>
                    <span className={styles.page_info}>
                      {allcarePage} / {totalPages}
                    </span>
                    <button
                      className={styles.page_btn}
                      disabled={allcarePage === totalPages}
                      onClick={() => setAllcarePage((p) => p + 1)}
                    >
                      <ChevronRight size={14} />
                    </button>
              ...
              <div key={team.team} className={styles.team_block}>
                ...
                >
                  ...
                  <div className={styles.team_meta}>
                    <span className={styles.team_amount}>
                      {formatAmount(team.paymentAmount)}
                    </span>
                  </div>
                </button>
                {isExpanded && (
                      ...
                      {team.managers.map((mgr) => (
                        <tr key={mgr.manager}>
                          ...
                          <td className={styles.td_right}>
                            {formatAmount(mgr.paymentAmount)}
                          </td>
                          <td className={styles.td_center}>
                            {mgr.completedCount}건
                          </td>
                          <td className={styles.td_center}>
                            {mgr.totalCount}건
                          </td>
    ...
    <div className={styles.tab_content}>
      ...
      <div className={styles.cert_summary_grid}>
        ...
        <div className={`${styles.summary_card} ${styles.card_purple}`}>
          ...
          <span className={styles.summary_value}>
            {formatAmount(data.total.avgAmount)}
          </span>
      ...
      {data.byDay.length === 0 ? (
        ...
        <div className={styles.team_block}>
          <div className={styles.cert_chart_header}>
            ...
            <span className={styles.cert_chart_sub}>
              {data.byDay.length}일 결제 발생
            </span>
          </div>
          <div className={styles.cert_chart_body}>
            {data.byDay.map((d) => (
              <div key={d.day} className={styles.cert_day_row}>
                ...
                <span className={styles.cert_day_amount}>
  ...
  return (
    <>
      <div className={styles.hero_amount_block}>
        <span className={styles.hero_amount_label}>
          {summary?.label ?? "이번 달 매출"}
        </span>
      ...
      <div className={styles.page_header}>
        <div className={styles.page_heading}>
          ...
          <p className={styles.page_subtitle}>
            {year}년 {month}월 기준 사업부별·담당자별 월 매출 현황
          </p>
        </div>
...
export default function NmsSalesPage() {
  ...
  const activeTab: TabKey =
    urlTab && availableTabs.includes(urlTab)
      ? urlTab
      : (availableTabs[0] ?? "stats");
      ...
      try {
        ...
        const revenueScope = Array.isArray(data.permissions)
          ? data.permissions.find(
              (permission: { section?: string; scope?: string }) =>
                permission.section === "revenues",
            )?.scope
          : null;
        ...
        const ownDivisions = Array.isArray(data.revenueOwnDivisions)
          ? data.revenueOwnDivisions.filter(
              (division: string): division is RevenueDivision =>
                division === "nms" ||
                division === "cert" ||
                division === "abroad",
            )
        ...
        const nextTabs: TabKey[] =
          ownDivisions.length > 1 ? ["stats", ...ownDivisions] : ownDivisions;
        setAvailableTabs(
          nextTabs.length > 0 ? Array.from(new Set(nextTabs)) : ["stats"],
        );
      } catch {
        // keep defaults
      }
  ...
  useEffect(() => {
    if (!availableTabs.includes(activeTab)) {
      router.replace(`/revenues/nms-sales?tab=${availableTabs[0] ?? "stats"}`, {
        scroll: false,
      });
    }
        ...
        <div className={styles.div_tabs}>
          {TABS.filter((tab) => availableTabs.includes(tab.key)).map((tab) => (
            <button
              ...
              onClick={() => {
                router.replace(`/revenues/nms-sales?tab=${tab.key}`, {
                  scroll: false,
                });
              }}
            >
