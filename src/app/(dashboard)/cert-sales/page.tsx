...
export default function CertSalesPage() {
  ...
  const fetchRows = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/cert-sales?${params.toString()}`);
      if (!res.ok) {
        setRows([]);
        return;
      }
  ...
  const updateRow = async (
    ...
    setRows((prev) =>
      prev.map((r) =>
        (r.sale_id ?? r.cert_application_id) ===
        (row.sale_id ?? row.cert_application_id)
          ? { ...r, ...patch }
          : r,
      ),
    );
    try {
      const payload: Record<string, unknown> = row.sale_id
        ...
        : row.cert_application_id
          ? { cert_application_id: row.cert_application_id, ...patch }
          : {
              ...patch,
      ...
      if (saved?.id && !row.sale_id) {
        setRows((prev) =>
          prev.map((r) =>
            (r.cert_application_id ?? "") === (row.cert_application_id ?? "")
              ? { ...r, sale_id: saved.id }
              : r,
          ),
        );
  ...
  // 환불 일괄 변경 — sale_id 없는 base 행도 자동 overlay 생성
  ...
  const handleDeleteRow = async (row: SalesRow) => {
    ...
    const params = row.sale_id
      ...
      : row.cert_application_id
        ? `cert_application_id=${encodeURIComponent(row.cert_application_id)}`
        : null;
    if (!params) {
      alert("삭제할 수 없는 행입니다.");
      return;
    }
    const res = await fetch(`/api/cert-sales?${params}`, { method: "DELETE" });
    if (!res.ok) {
      const err = await res.json().catch(() => ({}));
  ...
  // 행 식별자 — sale_id 있으면 sale: prefix, 없으면 app: prefix
  const rowKey = (r: SalesRow): string | null => {
    if (r.sale_id) return `sale:${r.sale_id}`;
    if (r.cert_application_id) return `app:${r.cert_application_id}`;
    return null;
  };
  ...
  const bulkDelete = async () => {
    if (selectedIds.size === 0) return;
    if (
      !confirm(`선택한 ${selectedIds.size}건을 삭제하시겠습니까?`)
    )
      return;
      ...
      ids.map((key) => {
        ...
        const params =
          type === "sale"
            ...
            : `cert_application_id=${encodeURIComponent(id)}`;
        return fetch(`/api/cert-sales?${params}`, { method: "DELETE" });
      }),
    );
    setSelectedIds(new Set());
    await fetchRows();
      ...
      <div className={styles.table_wrap}>
        {loading ? (
          ...
          <div className={styles.empty_state}>
            표시할 매출이 없습니다. 학점연계 신청에서 결제완료 건이 자동으로
            추가됩니다.
          </div>
        ) : (
              ...
              {filteredRows.map((r) => {
                ...
                return (
                  ...
                  >
                    ...
                    {/* 특이사항 — 클릭하면 팝업 */}
                    <td
                      className={`${styles.td} ${styles.td_center} ${styles.td_notes}`}
                    >
                      <button
                        type="button"
