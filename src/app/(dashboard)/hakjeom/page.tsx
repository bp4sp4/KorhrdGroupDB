...
const AGENCY_STATUS_STYLE: Record<
  AgencyStatus,
  { background: string; color: string }
> = {
...
const CAFE_CANONICAL_NAMES: string[] = [
...
const CAFE_ALIAS_MAP: Record<string, string> = {
  // 순광맘
...
function normalizeCafeName(raw: string): string {
  if (!raw) return raw;
  ...
  // 부분 일치 — alias가 raw 안에 포함되는 경우 (예: "강봉원 카페에서 들었어요")
...
const KNOWN_CAFE_KOREAN = new Set([
  ...Object.values(CAFE_NAMES),
  ...CAFE_CANONICAL_NAMES,
]);

function parseClickSource(
  source: string | null,
  customCafes: string[] = [],
...
// 맘카페 한글 이름 목록 (칩 버튼용)
// 칩 버튼/필터용 — 영문 ID 매핑된 이름 + 한글 표준명 33개 통합 (중복 제거, 순서 유지)
const CAFE_NAME_LIST = Array.from(
...
const COUNSEL_SUB: ConsultationStatus[] = [
  "상담완료-높음",
  "상담완료-중간",
  "상담완료-낮음",
];
      ...
      <div className={styles.detailModal}>
        ...
        <div className={styles.detailModalHeader}>
          <div className={styles.detailModalHeaderTop}>
            <div className={styles.detailModalHeaderLeft}>
              <div>
                ...
                <div className={styles.detailModalSubRow}>
                  <span className={styles.detailModalSub}>
            ...
            <button onClick={onClose} className={styles.detailModalCloseBtn}>
              ✕
            </button>
          </div>
        ...
        <div className={styles.detailModalBody}>
          ...
          {activeTab === "info" && (
            <>
                    ...
                    {customCafes.map((cafeName) => (
                      <span key={cafeName} className={styles.customItemWrap}>
                        ...
                        <button
                          ...
                          onClick={() => handleDeleteCafe(cafeName)}
                        >
                          ✕
                        </button>
                      </span>
                      ...
                      <>
                        <input
                          ...
                          onKeyDown={(e) => {
                            ...
                            if (e.key === "Escape") setShowCafeAdd(false);
                          }}
                          placeholder="카페 이름"
                          autoFocus
                        />
                      ...
                      <>
                        <input
                          ...
                          onKeyDown={(e) => {
                            ...
                            if (e.key === "Escape") setShowDanggeunAdd(false);
                          }}
                          placeholder="지역 이름"
                          autoFocus
                        />
              ...
              >
                ...
                <div className={styles.detailChipRow}>
                  {(
                    ...
                    const colors =
                      CONSULTATION_STATUS_STYLE[styleKey as ConsultationStatus];
                    return (
                      <button
                        key={s}
                        type="button"
                        ...
                        style={
                          isActive
                            ? {
                                ...
                                borderRadius: 20,
                                cursor: "pointer",
                                border: `2px solid ${colors?.color ?? "#333"}`,
              ...
              {/* 반응포인트 */}
              <div className={styles.detailChipSection}>
                <span className={styles.detailChipSectionLabel}>
                  반응포인트
                </span>
                <div className={styles.detailChipRow}>
                  {Object.keys(REACTION_POINT_MAP).map((parent) => {
                    const hasSelected = REACTION_POINT_MAP[parent].some((c) =>
                      editReactionPoint.includes(c),
                    );
                ...
                {editReactionPoint.length > 0 && (
                  <div
                    className={styles.detailChipRow}
                    style={{ marginTop: 6 }}
                  >
