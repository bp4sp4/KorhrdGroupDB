...
function ApprovalTable({
  ...
  const sorted = [...approvals].sort((a, b) => {
    ...
    if (sortField === "created_at") {
      ...
      va =
        (a.applicant as { display_name: string } | undefined)?.display_name ??
        "";
      vb =
        (b.applicant as { display_name: string } | undefined)?.display_name ??
        "";
    } else if (sortField === "document_number") {
      va = a.document_number ?? "";
      vb = b.document_number ?? "";
  ...
  const SortIcon = ({ field }: { field: SortField }) => {
    if (sortField !== field)
      return <ChevronsUpDown size={12} className={styles.th_sort_icon} />;
    return sortDir === "asc" ? (
      <ChevronDown
        size={12}
        className={styles.th_sort_icon_active}
    ...
    return (
      <div className={styles.table_empty}>
        <svg
          ...
          className={styles.table_empty_icon}
        >
      ...
      <tbody>
        {sorted.map((a) => (
          ...
          >
            ...
            {!compact && (
              <td className={styles.cell_category}>{a.category ?? "-"}</td>
            )}
            <td>
            ...
            {!compact && (
              <td className={styles.cell_applicant}>
                {(a.applicant as { display_name: string } | undefined)
                  ?.display_name ?? "-"}
              </td>
            )}
            <td className={styles.cell_doc_number}>
              {a.document_number ?? "-"}
            </td>
            <td>
  ...
  return (
    <div className={styles.steps_panel}>
      ...
      <div className={styles.steps_list}>
        {sorted.map((step) => {
          const isActive =
            step.step_number === currentStep && step.status === "PENDING";
          return (
            <div
              key={step.id}
            ...
            >
              ...
              <div className={styles.step_info}>
                <p className={styles.step_approver}>
                  {(step.approver as { display_name: string } | undefined)
                    ?.display_name ?? "-"}
                </p>
...
export default function ApprovalsPage() {
  ...
  const [users, setUsers] = useState<
    { id: string; display_name: string; department_id?: string | null }[]
  >([]);
  ...
  // 상세 뷰 패널 탭
  const [detailPanelTab, setDetailPanelTab] = useState<"steps" | "info">(
    "steps",
  );
  ...
  const [apmDraggingRowIdx, setApmDraggingRowIdx] = useState<number | null>(
    null,
  ); // 우측 행 드래그
  ...
  const [modalSelectedTemplate, setModalSelectedTemplate] =
  ...
  // 품의서 연동 (지출결의서 전용)
  const [linkedProposal, setLinkedProposal] = useState<LinkedProposal | null>(
    null,
  );
  const [proposalModalOpen, setProposalModalOpen] = useState(false);
  const [proposalCandidates, setProposalCandidates] = useState<Approval[]>([]);
  const [proposalLoadingCandidates, setProposalLoadingCandidates] =
    useState(false);

  // 연동 품의서 미리보기 팝업
  const [linkedProposalView, setLinkedProposalView] = useState<Approval | null>(
    null,
  );
  const [linkedProposalViewLoading, setLinkedProposalViewLoading] =
    useState(false);

  // 리스트 뷰 - 검색/페이지네이션/서브탭
  const [listPage, setListPage] = useState(1);
  const [listSearch, setListSearch] = useState("");
  const [listSearchField, setListSearchField] = useState<
  ...
  const fetchListData = useCallback(
    async (apiTab: "mine" | "pending" | "completed" | "reference") => {
      setListLoading(true);
  ...
  useEffect(() => {
    fetchHomeData();
    fetch("/api/management/approvals/templates")
      .then((r) => r.json())
      .then(setTemplates)
      .catch(() => {});
    fetch("/api/approvals/forms")
      .then((r) => r.json())
      .then((forms: ApprovalFormTemplate[]) => {
    ...
    fetch("/api/management/departments")
      ...
      .then(setDepartments)
      .catch(() => {});
    fetch("/api/management/bank-accounts")
      .then((r) => r.json())
      .then((data) => {
  ...
  useEffect(() => {
    ...
    setFormState((prev) => ({
      ...prev,
      department_id: String(myDepartmentId),
    }));
  ...
  const handleApprovalAction = async (
    ...
    setActioning(true);
    const res = await fetch(
      `/api/management/approvals/${selectedApproval.id}`,
      {
        method: "POST",
  ...
  const openProposalLinkModal = useCallback(async () => {
    ...
    const completed: Approval[] = completedRes.ok
    ...
    const isResolutionType = (a: Approval) => {
      const t = a.document_type.replace(/\s/g, "");
      return (
        t === "[결의서]지출결의서" ||
        t === "지출결의서(법인카드)" ||
        t === "지출결의서(이체요청)"
    ...
    const approvedProposals = completed.filter(
      (a) =>
        String(a.applicant_id) === String(myUserId) &&
        isProposalType(a) &&
        a.status === "APPROVED",
    );
    ...
    for (const a of [...mine, ...completed]) {
      ...
      const lp = (a.content as Record<string, unknown> | null)?.[
        "_linked_proposal"
      ] as LinkedProposal | undefined;
      if (lp?.id) linkedIds.add(String(lp.id));
    }
    setProposalCandidates(
      approvedProposals.filter((p) => !linkedIds.has(String(p.id))),
    );
    setProposalLoadingCandidates(false);
  ...
  const applyProposalLink = (p: Approval) => {
    ...
    const proposalFiles: AttachedFile[] = Array.isArray(rawAtt)
      ? (rawAtt as AttachedFile[])
      : [];
  ...
  const templatesByCategory = templates.reduce<
    Record<string, ApprovalTemplate[]>
  >((acc, t) => {
  ...
  const currentStepObj = sortedSteps.find(
    (s) =>
      s.step_number === selectedApproval?.current_step &&
      s.status === "PENDING",
  );
  const canAct =
  ...
  const formDynamicName = formIsDynamic
    ? (dynamicForms.find(
        (fo) => fo.document_type === formState.template?.document_type,
      )?.name ?? null)
    : null;
  ...
  const renderSidebar = () => (
    <aside className={styles.sidebar}>
      ...
      <div className={styles.sidebar_header}>
        <button className={styles.sidebar_title_btn} onClick={handleHomeClick}>
          전자결재
        </button>
      </div>
            ...
            if (item.key === "pending_approval") {
              count = pendingApprovals.filter((a) => {
                const myStep = a.steps?.find(
                  (s) => String(s.approver_id) === String(myUserId),
                );
              ...
              count = pendingApprovals.filter(
                (a) =>
                  Array.isArray(a.reference_ids) &&
                  a.reference_ids.map(String).includes(String(myUserId)),
              ).length;
            }
            return (
              <button
  ...
  const renderHomeView = () => (
    <div className={styles.view_home}>
      ...
      <section className={styles.home_section}>
        <div className={styles.home_section_title_row}></div>
  ...
  const getListViewTitle = () => {
    const found = SIDEBAR_GROUPS.flatMap((g) => g.items).find(
      (i) => i.key === activeMenuKey,
    );
  ...
  const searchedListApprovals = (() => {
    ...
    if (listSearch.trim()) {
      ...
      list = list.filter((a) => {
        if (listSearchField === "title")
          return a.title.toLowerCase().includes(q);
        if (listSearchField === "document_type")
          return a.document_type.toLowerCase().includes(q);
        if (listSearchField === "document_number")
          return (a.document_number ?? "").toLowerCase().includes(q);
        return false;
      });
    }
    return list;
  ...
  const totalListPages = Math.max(
    1,
    Math.ceil(searchedListApprovals.length / PAGE_SIZE),
  );
  const pagedListApprovals = searchedListApprovals.slice(
    ...
    <div className={styles.view_list}>
      ...
      <div className={styles.view_list_header}>
        ...
        <div className={styles.list_search_bar}>
          <select
            className={styles.list_search_select}
            value={listSearchField}
            onChange={(e) =>
              setListSearchField(
                e.target.value as "title" | "document_type" | "document_number",
              )
            }
          >
          ...
          />
          <button className={styles.list_search_btn}>
            <Search size={14} />
          </button>
        </div>
      </div>
      ...
      <div className={styles.list_pagination}>
        ...
        {Array.from({ length: totalListPages }, (_, i) => i + 1)
          .filter(
            (p) =>
              p === 1 || p === totalListPages || Math.abs(p - listPage) <= 2,
          )
          ...
          .map((p, i) =>
            p === "..." ? (
              <span key={`e${i}`} className={styles.page_ellipsis}>
                …
              </span>
            ) : (
              <button
                key={p}
                className={`${styles.page_btn} ${listPage === p ? styles.page_current : ""}`}
  ...
  const renderDetailView = () => {
    ...
    const applicantName =
      (selectedApproval.applicant as { display_name: string } | undefined)
        ?.display_name ?? "-";
    ...
    const handleResyncExpense = async () => {
      ...
      try {
        ...
        if (result?.synced) {
          alert(
            `손익 재반영이 완료되었습니다. 반영 건수: ${result.count ?? 0}건`,
          );
        } else {
        ...
        <div className={d.detail_body}>
          ...
          <div className={styles.doc_scroll_area}>
            <div className={styles.doc_paper}>
              {/* 문서 제목 */}
              <h2 className={styles.doc_title}>
                {selectedApproval.document_type}
              </h2>
              ...
              <div className={styles.doc_header_area}>
                ...
                <table className={styles.doc_approval_box}>
                  <tbody>
                    <tr>
                      <td className={styles.doc_approval_label} rowSpan={3}>
                        결재
                      </td>
                      {sortedSteps.length > 0 ? (
                        sortedSteps.map((step) => (
                          <td
                            key={step.id}
                            className={styles.doc_approval_name_cell}
                          >
                            {(
                              step.approver as
                                | { display_name: string }
                        ...
                        <td className={styles.doc_approval_empty_cell}>
                          미지정
                        </td>
                      )}
                    </tr>
                    <tr>
                      {sortedSteps.length > 0 ? (
                        sortedSteps.map((step) => (
                          <td
                            key={step.id}
                            className={styles.doc_approval_status_cell}
                          >
                            {step.status === "APPROVED"
                              ? "승인"
                              : step.status === "REJECTED"
              ...
              {(() => {
                const raw = (
                  selectedApproval.content as Record<string, unknown>
                )?.["_linked_proposal"];
                ...
                return (
                  <table className={styles.linkedProposalTable}>
                    ...
                    <tbody>
                      <tr>
                        <td className={styles.linkedProposalLabelCell}>
                          문서번호
                        </td>
                        <td className={styles.linkedProposalValueCell}>
                          <button
                            ...
                            onClick={async () => {
                              ...
                              const res = await fetch(
                                `/api/management/approvals/${lp.id}`,
                              );
                              if (res.ok) {
                          ...
                          <span className={styles.linkedProposalSubtitle}>
                            {lp.title}
                          </span>
                        </td>
                      </tr>
              ...
              {(() => {
                ...
                if (!cfg) return null;
                const content = selectedApproval.content as Record<
                  string,
                  unknown
                >;
                return (
              ...
              {(() => {
                ...
                const files: AttachedFile[] = Array.isArray(raw)
                  ? (raw as AttachedFile[])
                  : [];
                      ...
                      {files.map((file, i) => (
                        <div key={i} className={d.doc_file_item}>
                          ...
                          <div className={d.doc_file_info}>
                            ...
                            <span className={d.doc_file_size}>
          ...
          {/* 지출결의서(법인카드/이체요청): 우측 통장 패널 */}
          {(() => {
            const cfg = getDocTemplate(selectedApproval, dynamicConfigs);
            if (
              cfg?.id !== "expense-resolution-card" &&
              cfg?.id !== "expense-resolution-transfer"
            )
              return null;
            const content = selectedApproval.content as Record<string, unknown>;
            const accountId = String(content["bank_account_id"] ?? "");
            const acc = bankAccounts.find((b) => b.id === accountId);
            const isCard = cfg?.id === "expense-resolution-card";
            return (
              <aside className={styles.doc_proof_aside}>
                <ExpenseProofPanel
                  title={
                  ...
                  hideCardLast4={!isCard}
                  externalAccount={
                    acc
                      ? {
                          bankCode: acc.bank_code ?? "",
                          accountNumber: acc.account_number,
                          accountName: acc.account_holder ?? acc.bank_name,
                          memo: acc.memo ?? null,
                        }
                      : null
                  }
                  cardItems={isCard ? parseCardItems(content) : []}
                />
              </aside>
            );
          })()}
          ...
          {lightboxUrl && (
            <div
              className={d.lightbox_overlay}
              onClick={() => setLightboxUrl(null)}
            >
              <button
                className={d.lightbox_close}
                onClick={() => setLightboxUrl(null)}
              >
                <X size={20} />
              </button>
              ...
              <div className={f.template_grid}>
                {(templatesByCategory[selectedCategory] ?? []).map((tpl) => (
                  ...
                  >
                    ...
                    <span className={f.template_card_name}>
                      {tpl.document_type}
                    </span>
                    <span className={f.template_card_category}>
                      {tpl.category}
                    </span>
                  </button>
                ))}
  ...
  const renderNewFormView = () => {
    ...
    // 현재 로그인 유저 이름
    const myUserName =
      users.find((u) => String(u.id) === String(myUserId))?.display_name ?? "-";
    ...
    return (
      <div className={f.view_new_form}>
        ...
        <div className={f.new_form_header}>
          <div>
            <p className={f.new_form_category}>
              {formState.template?.category}
            </p>
            <h2 className={f.new_form_title}>
              {formHeadingTitle}
              {editingDraftId && (
                <span className={f.draft_editing_badge}>임시저장 편집 중</span>
              )}
            </h2>
          </div>
          <div
        ...
        <div className={styles.doc_with_aside}>
                ...
                <table className={styles.doc_info_table}>
                  <tbody>
                    ...
                    <tr>
                      ...
                      <td>
                        {departments.find(
                          (d) => String(d.id) === formState.department_id,
                        )?.name ??
                          departments.find(
                            (d) => String(d.id) === (myDepartmentId ?? ""),
                          )?.name ??
                          "-"}
                      </td>
                    </tr>
                          ...
                          <div className={styles.doc_reference_cell}>
                            {formState.reference_ids.map((uid, idx) => {
                              ...
                              return (
                                <span
                                  key={idx}
                                  className={styles.doc_reference_chip}
                                >
                                  {u?.display_name ?? uid}
                                  <button
                                    ...
                                    onClick={() =>
                                      setFormState({
                                        ...formState,
                                        reference_ids:
                                          formState.reference_ids.filter(
                                            (_, i) => i !== idx,
                                          ),
                                      })
                                    }
                    ...
                    <tr>
                      ...
                      <td>
                        <span className={styles.doc_info_input}>
                          {formState.title}
                        </span>
                      </td>
                    </tr>
                        ...
                        {linkedProposal ? (
                          <>
                            <button
                              ...
                              onClick={async () => {
                                setLinkedProposalViewLoading(true);
                                setLinkedProposalView({
                                  id: linkedProposal.id,
                                } as Approval);
                                const res = await fetch(
                                  `/api/management/approvals/${linkedProposal.id}`,
                                );
                                if (res.ok) {
                                  setLinkedProposalView(await res.json());
                                } else {
                            ...
                            >
                              {linkedProposal.document_number ??
                                "문서번호 없음"}
                            </button>
                            <span className={styles.linkedProposalSubtitle}>
              ...
              {formTemplateConfig?.supportsAttachments && (
                <div className={styles.doc_file_attach_section}>
                  <div className={f.doc_file_attach_label_row}>
                    <span className={styles.doc_file_attach_title}>
                      파일첨부
                    </span>
                  </div>
                  <div
                    className={`${f.doc_file_attach_box} ${uploadingFiles ? f.doc_file_attach_uploading : ""}`}
                  >
                    <label
                    ...
                    >
                      <input
                        type="file"
                        multiple
                        className={f.doc_file_attach_input}
                      ...
                      <span>
                        {uploadingFiles ? (
                          ...
                          <>
                            이 곳에 파일을 드래그 하세요. 또는{" "}
                            <span className={f.doc_file_select_link}>
                              파일선택
                            </span>
                            {attachedFiles.length > 0 && (
                              <span className={f.doc_file_total_size}>
                                {" "}
                                (
                                {(
              ...
              <div className={f.doc_approver_section}>
                ...
                <div className={f.doc_approver_list}>
                  ...
                  <div className={f.doc_approver_list_item}>
                    <div className={f.doc_approver_list_name}>
                      {myUser?.display_name ?? "-"}
                    </div>
                    {myDept && (
                      <div className={f.doc_approver_list_dept}>
                        {myDept.name}
                      </div>
                    )}
                  ...
                  {formState.approver_ids.map((uid, idx) => {
                    ...
                    const dept = departments.find(
                      (d) => String(d.id) === String(u?.department_id),
                    );
                    return (
                      <div
                        key={idx}
                      ...
                      >
                        ...
                        <div className={f.doc_approver_list_role}>
                          결재 예정
                        </div>
                        <button
                          ...
                          onClick={() =>
                            setFormState({
                              ...formState,
                              approver_ids: formState.approver_ids.filter(
                                (_, i) => i !== idx,
                              ),
                            })
                          }
          ...
          {formTemplateConfig?.id === "corporate-card" && (
            <aside className={styles.doc_proof_aside}>
              <ExpenseProofPanel
                cardItems={parseCardItems(
                  formState.content as Record<string, unknown>,
                )}
                onAddItem={(picked) => {
                ...
                onRemoveItem={(picked) => {
                  ...
                  const idx = list.findIndex(
                    (r) =>
                      r.date === picked.date &&
                      Number(r.amount) === Number(picked.amount),
                  );
                  if (idx < 0) return;
                  list.splice(idx, 1);
                  handleContentChange("card_items", JSON.stringify(list));
            ...
            (() => {
              ...
              return (
                <aside className={styles.doc_proof_aside}>
                  <ExpenseProofPanel
                    title={
                      acc?.account_holder || acc?.bank_name || "사업부 통장"
                    }
                    externalAccount={
                      acc
                        ? {
                    ...
                    cardItems={parseCardItems(content)}
                    onAddItem={(picked) => {
                      const raw = content["card_items"];
                      let list: Array<Record<string, string>> = [];
                      try {
                      ...
                      if (emptyIdx >= 0) list[emptyIdx] = newRow;
          ...
          {formTemplateConfig?.id === "expense-resolution-transfer" &&
            (() => {
              ...
              return (
                <aside className={styles.doc_proof_aside}>
                  <ExpenseProofPanel
                    title={acc?.account_holder || acc?.bank_name || "출금 통장"}
                    hideCardLast4
                    externalAccount={
                      acc
                        ? {
        ...
        {previewOpen && (
          <div
            className={f.preview_overlay}
            onClick={() => setPreviewOpen(false)}
          >
            <div
              className={f.preview_modal}
              onClick={(e) => e.stopPropagation()}
            >
                ...
                <div className={f.preview_modal_header_actions}>
                  <button
                    ...
                    onClick={() => {
                      ...
                      setOrgSearch("");
                      setExpandedDepts(
                        new Set(departments.map((d) => String(d.id))),
                      );
                      setApproverModalOpen(true);
                    }}
                  >
                  ...
                  <button
                    className={f.preview_modal_close}
                    onClick={() => setPreviewOpen(false)}
                  >
                    <X size={18} />
                  </button>
  ...
  const filteredTemplates = modalSearch.trim()
    ? templates.filter(
        (t) =>
          t.document_type.includes(modalSearch.trim()) ||
          t.category.includes(modalSearch.trim()),
      )
    : modalCategory
      ? (templatesByCategory[modalCategory] ?? [])
      : [];

  const templateModal =
    templateModalOpen && typeof document !== "undefined"
      ? createPortal(
          <div
                      ...
                      : Object.keys(templatesByCategory).map((cat) => (
                          <div key={cat} className={styles.tplTreeGroup}>
                            ...
                            >
                              <ChevronRight
                                size={13}
                                className={`${styles.tplChevron} ${modalCategory === cat ? styles.tplChevronOpen : ""}`}
                              />
                              {cat}
                            </button>
                            {modalCategory === cat &&
                  ...
                  {modalSelectedTemplate ? (
                    <div className={styles.tplDetailRows}>
                      <div className={styles.tplDetailRow}>
                        ...
                        <span className={styles.tplDetailValue}>
                          {modalSelectedTemplate.document_type}
                        </span>
                      </div>
                      <div className={styles.tplDetailRow}>
                        <span className={styles.tplDetailLabel}>카테고리</span>
                        <span className={styles.tplDetailValue}>
                          {modalSelectedTemplate.category}
                        </span>
                      </div>
                      <div className={styles.tplDetailRow}>
                      ...
                      <div className={styles.tplDetailRow}>
                        ...
                        <div className={styles.tplDetailValue}>
                          <select
                            ...
                            onChange={(e) =>
                              setModalDepartmentId(e.target.value)
                            }
                          >
                    ...
                    <p className={styles.tplDetailEmpty}>
                      좌측에서 양식을 선택하세요.
                    </p>
                  )}
                </div>
  ...
  const proposalModal =
    proposalModalOpen && typeof document !== "undefined"
      ? createPortal(
          <div
            className={styles.tplOverlay}
            onMouseDown={(e) => {
            ...
            >
              ...
              <div className={styles.proposalModalBody}>
                {proposalLoadingCandidates ? (
                  ...
                  <p className={styles.proposalModalEmpty}>
                    연동 가능한 승인 완료 품의서가 없습니다.
                  </p>
                ) : (
                          ...
                          >
                            ...
                            {(vendor || amount) && (
                              <p className={styles.proposalItemMeta}>
                                ...
                                {vendor && amount && (
                                  <span className={styles.proposalItemDot}>
                                    ·
                                  </span>
                                )}
  ...
  const linkedProposalModal =
    linkedProposalView && typeof document !== "undefined"
      ? createPortal(
          <div
            className={styles.tplOverlay}
            onMouseDown={(e) => {
              if (e.target === e.currentTarget) setLinkedProposalView(null);
            }}
          >
            <div className={styles.linkedProposalViewModal}>
              ...
              <div className={styles.linkedProposalViewBody}>
                {linkedProposalViewLoading ||
                !linkedProposalView.document_type ? (
                  <p className={styles.proposalModalEmpty}>불러오는 중...</p>
                ) : (
                  <>
                    <h3 className={styles.linkedProposalViewTitle}>
                      {linkedProposalView.title}
                    </h3>
                    {(() => {
                      const cfg = getDocTemplate(
                        linkedProposalView,
                        dynamicConfigs,
                      );
                      if (!cfg)
                      ...
                      const content = (linkedProposalView.content ??
                    ...
                    {(() => {
                      const raw = (
                        linkedProposalView.content as Record<string, unknown>
                      )?.["_attachments"];
                      const files: AttachedFile[] = Array.isArray(raw)
                        ? (raw as AttachedFile[])
                        : [];
                      ...
                      return (
                        <div className={styles.linkedProposalViewFiles}>
                          <p className={styles.linkedProposalViewFilesLabel}>
                            품의서 첨부파일
                          </p>
                          {files.map((file, i) => (
                            <div
                            ...
                            >
                              ...
                              <span
                                className={styles.linkedProposalViewFileSize}
                              >
                                ({(file.size / 1024).toFixed(0)}KB)
                              </span>
                              <a
  ...
  const myDept = departments.find(
    (d) => String(d.id) === String(myUser?.department_id),
  );

  const usersByDept = departments
    .map((dept) => ({
      dept,
  ...
  const ungrouped = users.filter(
    (u) =>
      !u.department_id ||
      !departments.find((d) => String(d.id) === String(u.department_id)),
  );
  ...
  const approverModal =
    approverModalOpen && typeof document !== "undefined"
      ? createPortal(
          <div
            className={styles.apmOverlay}
            onMouseDown={(e) => {
              if (e.target === e.currentTarget) setApproverModalOpen(false);
            }}
          >
                      ...
                      searchedUsers.map((u) => {
                        ...
                        return (
                          <button
                            ...
                            onDragStart={(e) => {
                              e.dataTransfer.setData("apmUserId", String(u.id));
                              e.dataTransfer.effectAllowed = "copy";
                              apmIsDragging.current = true;
                            }}
                            onDragEnd={() => {
                              setTimeout(() => {
                                apmIsDragging.current = false;
                              }, 0);
                            }}
                            onClick={() => {
                              if (!added && !apmIsDragging.current)
                                setApproverModalDraft((prev) => [
                                  ...prev,
                                  String(u.id),
                                ]);
                            }}
                          >
                            <div className={styles.apmUserAvatar}>
                              {u.display_name[0]}
                            </div>
                            <div className={styles.apmUserInfo}>
                              <span className={styles.apmUserName}>
                                {u.display_name}
                              </span>
                              {dept && (
                                <span className={styles.apmUserDept}>
                                  {dept.name}
                                </span>
                              )}
                            </div>
                            ...
                            <button
                              ...
                              onClick={() =>
                                setExpandedDepts((prev) => {
                                  ...
                                  s.has(String(dept.id))
                                    ? s.delete(String(dept.id))
                                    : s.add(String(dept.id));
                                  return s;
                                })
                              }
                            >
                              <ChevronDown
                                size={14}
                                className={`${styles.apmChevron} ${expandedDepts.has(String(dept.id)) ? styles.apmChevronOpen : ""}`}
                              />
                              {dept.name}
                              <span className={styles.apmDeptCount}>
                                {members.length}
                              </span>
                            </button>
                            {expandedDepts.has(String(dept.id)) &&
                              members.map((u) => {
                                const added = approverModalDraft.includes(
                                  String(u.id),
                                );
                                return (
                                  <button
                ...
                >
                  <table className={styles.apmTable}>
                    ...
                    <tbody>
                      ...
                      <tr>
                        <td className={styles.apmArrow}>
                          <ChevronsRight size={16} />
                        </td>
                        <td>
                          <span className={styles.apmTypeLabel}>기안</span>
                        </td>
                      ...
                      {approverModalDraft.length === 0 ? (
                        ...
                        >
                          ...
                          <td
                            colSpan={5}
                            className={`${styles.apmEmptyRow} ${apmDropBefore === 0 ? styles.apmEmptyRowDrop : ""}`}
                          >
                            {apmDropBefore === 0
                              ? "여기에 놓으세요"
                        ...
                        <>
                            ...
                            return (
                              <tr
                                ...
                                onDrop={(e) => {
                                  ...
                                  e.stopPropagation();
                                  const userId =
                                    e.dataTransfer.getData("apmUserId");
                                  const rowIdxStr =
                                    e.dataTransfer.getData("apmRowIdx");
                                  if (
                                    userId &&
                                    !approverModalDraft.includes(userId)
                                  ) {
                              ...
                              >
                                ...
                                <td>
                                  <span className={styles.apmTypeLabel}>
                                    결재
                                  </span>
                                </td>
                          ...
                          <tr
                            ...
                            onDragOver={(e) => {
                              e.preventDefault();
                              setApmDropBefore(approverModalDraft.length);
                            }}
                            onDragLeave={(e) => {
                              if (
                                !e.currentTarget.contains(
                ...
                <div className={styles.apmFooterRight}>
                  <button
                    ...
                    onClick={() => {
                      setFormState({
                        ...formState,
                        approver_ids: approverModalDraft,
                      });
  ...
  const refModal =
    refModalOpen && typeof document !== "undefined"
      ? createPortal(
          <div
            className={styles.apmOverlay}
            onMouseDown={(e) => {
              if (e.target === e.currentTarget) setRefModalOpen(false);
            }}
          >
            <div className={styles.apmModal}>
              <div className={styles.apmHeader}>
              ...
              <div className={styles.apmBody}>
                          ...
                          members.map((u) => {
                            ...
                            return (
                              <button
                                ...
                                onClick={() => {
                                  if (!added && !refIsDragging.current)
                                    setRefModalDraft((prev) => [
                                      ...prev,
                                      String(u.id),
                                    ]);
                                }}
                ...
                <div
                  ...
                  onDrop={(e) => {
                    e.preventDefault();
                    const userId = e.dataTransfer.getData("apmUserId");
                    if (userId && !refModalDraft.includes(userId))
                      setRefModalDraft((prev) => [...prev, userId]);
                  }}
                >
                  <table className={styles.apmTable}>
                    <thead>
                    ...
                    <tbody>
                      {refModalDraft.length === 0 ? (
                        <tr>
                          ...
                          <td colSpan={3} className={styles.apmEmptyRow}>
                            드래그하여 참조자를 추가할 수 있습니다.
                          </td>
                        </tr>
                      ) : (
  ...
  return (
    <div className={styles.page_layout}>
      {renderSidebar()}
      <main className={styles.main_area}>{renderMainContent()}</main>
      {templateModal}
      {approverModal}
      {refModal}
      {proposalModal}
