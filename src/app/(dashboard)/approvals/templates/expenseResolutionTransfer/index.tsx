...
export const EXPENSE_RESOLUTION_TRANSFER_FIELDS: FieldDef[] = [
  ...
  { key: 'account_number',   label: '입금 계좌번호(-제외)',  type: 'text',     required: true },
  { key: 'account_holder',   label: '입금 예금자명',        type: 'text',     required: true },
...
export function ExpenseResolutionTransferBody({ content, onChange, departments = [], bankAccounts = [] }: DocBodyProps) {
  const ro = !onChange
  ...
  return (
    <>
      ...
      <table className={styles.table_main}>
        <thead>
          <tr>
            <th colSpan={4} className={styles.th_title}>출금(자사) 정보</th>
          </tr>
        </thead>
        <tbody>
          <tr>
      ...
      {/* ── 입금요청 상세 ── */}
      <p className={styles.section_title}>입금요청 상세내용</p>
      <table className={styles.table_main}>
        <tbody>
          {/* 상호명 / 거래처 전화번호 */}
          <tr>
