...
export const AFFILIATE_PAYMENT_FIELDS: FieldDef[] = [
  { key: 'category',            label: '구분',            type: 'text' },
  { key: 'partner_name',        label: '제휴기관명',       type: 'text', required: true },
  ...
  { key: 'tax_invoice_issued',  label: '선발행 유무',      type: 'text' },
  { key: 'tax_invoice_reason',  label: '선발행 사유',      type: 'text' },
  { key: 'deduction_months',    label: '공제 적용 월',     type: 'text' },
...
export function AffiliatePaymentBody({ content, onChange, departments = [] }: DocBodyProps) {
  const ro = !onChange
  const costType = v(content, 'cost_type') || 'tax_invoice'
  const taxIssued = v(content, 'tax_invoice_issued') || 'O'
  ...
  return (
    <>
      ...
      <table className={styles.table_main}>
        <thead>
          <tr>
            <th colSpan={6} className={styles.th_title}>입금요청 상세내용</th>
          </tr>
        </thead>
        <tbody>
          {/* 구분 / 제휴기관명 / 관계자 연락수단 */}
          <tr>
            ...
            <td className={styles.label_cell}>제휴기관명</td>
            <td className={styles.value_cell}>
              {ro ? (
          ...
          <tr>
            ...
            <td colSpan={2} className={styles.value_cell_num}>
              {ro ? (
          ...
          <tr>
            ...
            <td colSpan={5} className={styles.value_cell}>
              {ro ? (
                ...
                <div className={styles.radio_group}>
                  {COST_TYPE_OPTIONS.map(opt => (
                    <label key={opt.value} className={styles.radio_label}>
                      <input type="radio" name="affiliate_cost_type"
      ...
      <table className={own.table_section}>
        <thead>
          <tr>
            <th colSpan={2} className={own.section_title}>※ 세금계산서 선택 시 선발행 유무</th>
          </tr>
        </thead>
        <tbody>
          <tr>
            <td className={own.radio_cell}>
              {ro ? (
                <span>{taxIssued === 'O' ? 'O (선발행 증명 첨부)' : 'X (오른쪽 사유 작성)'}</span>
              ) : (
                <div className={styles.radio_group}>
                  <label className={styles.radio_label}>
                    <input type="radio" name="tax_issued"
                      value="O" checked={taxIssued === 'O'}
                      onChange={() => onChange!('tax_invoice_issued', 'O')} />
                    O (선발행 증명 첨부)
                  </label>
                  <label className={styles.radio_label}>
                    <input type="radio" name="tax_issued"
                      value="X" checked={taxIssued === 'X'}
                      onChange={() => onChange!('tax_invoice_issued', 'X')} />
                    X (오른쪽 사유 작성)
                  </label>
                </div>
              )}
            </td>
            <td className={own.reason_cell}>
              {ro ? (
                <span>{v(content, 'tax_invoice_reason')}</span>
              ) : (
                <input type="text" className={styles.input_full}
                  value={v(content, 'tax_invoice_reason')} placeholder=""
                  disabled={taxIssued !== 'X'}
                  onChange={(e) => onChange!('tax_invoice_reason', e.target.value)} />
              )}
            </td>
          </tr>
        </tbody>
      ...
      <table className={own.table_section}>
        <thead>
          <tr>
            <th className={own.section_title}>공제 적용 월 선택</th>
          </tr>
        </thead>
        <tbody>
          <tr>
      ...
      <div className={own.notes}>
        ...
        <p>※ 상호명(거래처명)은 사업자등록증에 기재되어 있는 거래처명을 뜻합니다. (제휴기관명과 다를 수 있음)</p>
        <p>※ 선택한 공제 적용 월의 급여명세서에 해당 금액이 반영되어 공제 처리됩니다.</p>
        <p>(재계약 건의 경우, 전 계약과 동일한 공제 적용 월 중복 선택은 허용되지 않습니다)</p>
      </div>
    </>
  )
}
