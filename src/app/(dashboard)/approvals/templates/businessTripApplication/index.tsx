...
export const BUSINESS_TRIP_APPLICATION_FIELDS: FieldDef[] = [
  ...
  { key: 'trip_plan',     label: '출장 계획',   type: 'textarea' },
  { key: 'expense_items', label: '예상 경비',   type: 'text' },
]
...
export function BusinessTripApplicationBody({ content, onChange }: DocBodyProps) {
  const ro = !onChange
  ...
  return (
    <>
                  ...
                  <td key={colIdx} className={styles.td_person}>
                    {ro ? (
                      ...
                      <div className={styles.person_item}>
                        <input type="text" className={styles.person_input}
                          value={person} placeholder="홍길동 / 대리"
      ...
      {/* ── 출장 계획 ── */}
      <p className={`${styles.section_title} ${styles.section_title_mt}`}>출장 계획</p>
      <table className={styles.table_detail}>
        <tbody>
          <tr>
            <td className={`${styles.label_cell} ${styles.label_cell_top}`}>출장 계획</td>
            <td className={styles.value_cell}>
              {ro ? (
                <span className={styles.pre_wrap}>{v(content, 'trip_plan') || '-'}</span>
              ) : (
                <textarea className={styles.textarea_full}
                  value={v(content, 'trip_plan')}
                  placeholder="출장 일정 및 계획을 입력하세요"
                  onChange={(e) => onChange!('trip_plan', e.target.value)} />
              )}
            </td>
          </tr>
        </tbody>
      ...
      {/* ── 예상 경비 ── */}
      <p className={`${styles.section_title} ${styles.section_title_mt}`}>예상 경비</p>
      <table className={styles.table_expense}>
        <thead>
          <tr>
            <th className={styles.expense_th_name}>구분</th>
            <th className={styles.expense_th_amt}>예상 금액</th>
            <th className={styles.expense_th_note}>비고</th>
          </tr>
        </thead>
        <tbody>
      ...
      <p className={styles.note}>※ 예상 경비는 실제 집행 경비와 다를 수 있습니다</p>
    </>
  )
}
